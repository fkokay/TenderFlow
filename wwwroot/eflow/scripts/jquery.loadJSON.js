...
* Orjinal Version:  1.2.3.
* Version:          1.2.4.
* Author:           Jovan Popovic 
* Modified By:      Sinan Gündüz  
* Copyright 2011 Jovan Popovic, all rights reserved.
*
* Yapılan modifiyelerin üzerine * Modify * yazılmıştır */
...
(function ($) {
    $.fn.loadJSON = function (obj, options) {
        function setElementValue(element, value, name) {
            ...
            type = element.className === "TaskButtons" ? "TaskButtons" : type;
            type = element.className === "TID" ? "TID" : type;
            if (type == null)
                return;
            ...
            if ($(element).hasClass("datetime")) {
                var dateon = moment(value);
                if ($(element).hasClass("DueDate")) {
                    var now = moment(new Date());
                    if (now > dateon) {
                        value = '<span class="overdue">' + dateon.format(DateFormat) + '</span>';
                    }
                    else {
                        value = dateon.format(DateFormat);
                    }
                }
                ...
                else if ($(element).hasClass("Expires")) {
                    if (value == "\/Date(-62135596800000)\/") {
                        value = "-";
                    }
                    else {
            ...
            switch (type) {
                ...
                case 'taskbuttons':
                    $.each(value, function (index, item) {
                        ...
                        $(element).find("ul").append('<li class="pad"><button type="button" id="' + item.ButtonID + '" targetId="' + item.TargetActID + '" class="btn btn-primary TaskAction" name="TaskAction" style="width:max-content; background:' + item.BackGround + ';color:' + item.TextColor + ';" value="' + item.ButtonName + '" > ' + item.ButtonName + '</button></li>');

                    });
                    break;
                case 'tid':
                    $(element).append('<input type="checkbox" class="icheck-me" data-skin="square" data-color="aero" data-id="' + value + '" />');
                    break;
                case 'submit':
                case 'button':
                default:
                        try {
                            /* Modify 3 */
                            if ($(element).hasClass("BackColor")) {
                                if (value == "FFFFFF") {
                                    value = "EEE";
                                }
                               
                                $(element).html('<div class="tflag" style="background-color: #' + value + '"> </div>');
        ...
        function browseJSON(obj, element, name) {
            ...
            // branch
                ...
                /* Modify 2*/
                else if ($(element).hasClass("taskflag")) {
                    $(element).css("background-color", "#" + obj.BackColor);
                    $(element).css("color", "#" + obj.TextColor);
                }
                else if ($(element).prop("tagName").toLowerCase() == "tr" && $(element).hasClass("ID")) {
                    $(element).attr("rowid", obj.ID);
                }
                /* Modify 2 END*/
                for (var prop in obj) {
                    if (prop == null)
                ...
                if (element.length > 0 && element[0].tagName == "SELECT" && element[0].type == "select-multiple") {
                    ...
                    if (arrayElements.length > 0) {//if there are rel=[index] elements populate them instead of iteration
                        ...
                        if (element.selector === ".TaskButtons") {
                            setElementValue(element[0], obj, name);
                        } else {
