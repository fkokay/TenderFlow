var GroupFilter = (function (document, window, $) {
    ...
    function PrintGlobalVariables() {
        ...
        var globalVariables = {
            ...
            DataMembers: JSON.stringify(DataMembers),
    ...
    function GetMemberData() {
        $.ajax({
            type: 'GET',
            dataType: "json",
            url: '/Group/GetAllMembers',
            success: function (data) {

                $.each(data.data, function (key, entry) {

                    var model = { ID: entry.USERID, DISPLAYNAME: entry.UserName }

                    DataMembers.push(model);
                })
            }
        }).done(function () {
    ...
    function InitFilterDefault() {
        ...
        $('#select2-SelectedGroupElementId-results').parents('.select2-container').hide();

        //Final Form, All Search Boxes Hidden
        $('#groupCriteriaSearchArea').hide();
    }
    ...
    function BindCriteriaData() {
        ...
        let dropdown = $('#SelectedGroupElementId');

        dropdown.empty();
        dropdown.append('<option value="">&nbsp;</option>');

        dropdown.append($('<option type=' + 'group_no' + '></option>').attr('value', 1).text(EFlang.FilterGroupNo));
        dropdown.append($('<option type=' + 'group_name' + '></option>').attr('value', 2).text(EFlang.FilterGroupName));
        dropdown.append($('<option type=' + 'group_member' + '></option>').attr('value', 3).text(EFlang.FilterGroupMember));
        dropdown.append($('<option type=' + 'spec_code' + '></option>').attr('value', 4).text(EFlang.FilterGroupSpecCode));
    }
    ...
    function ShowHideSearchControls(selectedElementType, isRe0pen) {
        ...
        if (selectedElementType == 2 || selectedElementType == 4) {
            $('.string-search').show();
    ...
    function StartAddFilter() {
        ...
        setTimeout(function () {
            $('#select2-SelectedGroupElementId-results').parents('.select2-container').show();
            $('.kt-header__topbar-item #p-search-group-criteria-panel').removeClass('hide');
            $('.kt-header__topbar-item #p-search-group-criteria-panel').addClass('show');
            $('#SelectedGroupElementId').select2('open');
        }, 250);
    }
    ...
    function CloseCriteriaSelectAreaPanel() {
        ...
        $('#select2-SelectedGroupElementId-results').parents('.select2-container').hide();
        $('.kt-header__topbar-item #p-search-group-criteria-panel').removeClass('show');
        $('.kt-header__topbar-item #p-search-group-criteria-panel').addClass('hide');
        $('#btnAddFilter').show();
    }
    ...
    function GetSearchResult(id, type, filterText, matchType) {
        ...
        $.ajax({
            type: 'GET',
            data: filter,
            url: '/Admin/GroupSearchResultIDsByCriteria',
            success: function (data) {
    ...
    function DismissSearchParam(index) {
        ...
        if (postModelArray.length > 0) {
            ...
            $.ajax({
                ...
                traditional: true,
                url: '/Admin/GroupFilterDismissSearchParam',
                success: function (data) {
    ...
    function ClearAllProcessCriteriaSearchAreaInputs() {
        $('#groupCriteriaSearchArea').find('input:text').val('');
    ...
    function Events() {
        ...
        $('#SelectedGroupElementId').on('change', function () {
            ...
            var dataElementText = $("#SelectedGroupElementId :selected").text();
        ...
        $("#btnSearchApply").click(function () {
            ...
            if (SelectedElementId == 2 || SelectedElementId == 4) {
